...
function CausalShoes({ products = causalShoes }) {
  // Main issue: the hook was imported and called incorrectly.
  // `useNavigate` must be used, not `useNavigation`, and the function name is `useNavigate()`.
  // Also the route path string had an extra space before the encoded heading,
  // which would create an invalid URL and prevent the detail page from opening.
        ...
        {products.map((product) => {
          return (
            <div
              key={product.id}
              onClick={() => navigate(
                `/product/causal-shoes/${encodeURIComponent(product.heading)}`,
                { state: { product } }
              )}
            ...
            >
              <div className='relative h-70 object-cover w-full overflow-hidden rounded-[20px_20px_0_0]'>
                ...
                />
                <div className='absolute left-2 top-3 rounded-2xl bg-white/10 px-2.5 py-0.5 text-black border border-dashed'>
                  Causal
                </div>
              </div>
